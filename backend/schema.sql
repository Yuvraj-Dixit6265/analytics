...
-- Recurring "email this report" scheduling lives in WorkFlow (report_schedules
-- table there), not here — this service only renders reports and sends them
-- on request (/api/r/<key>/<token>/email) or on WorkFlow's behalf
-- (/api/internal/reports). See WorkFlow/services/report_schedules.py.
